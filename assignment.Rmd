...
<span style="color:blue">
**A: The plot line with quadratic time trend applied is seemed to have no evidence for slowing yield growth instead it shows a similar trend to the previous graph.**</span>
...
<span style="color:blue">
**A: The result of analysis between temperature and yield for all counties of Iowa during 1981~2020 shows a slow negative correlation, blue dashed line, contrary to the one of Question #3. And it shows that there is no significant difference in yield result due to temperature coefficients change(Tmax, Tmaxsq).** </span>
...
ggplot(Tmax_yield_all_years, mapping = aes(x=meantmax, y=yield)) +
  geom_point() +
  geom_line (mapping = aes(x=meantmax, y=fitted), col='red') +
  theme_bw() +
  labs(x='Tmax (°C)', y='Corn yield [bu/ac]',
       title='Relation between temperature and yield(Tmax)')+
...
ggplot(Tmax_yield_all_years, mapping = aes(x=meantmax, y=yield)) +
  ...
  theme_bw() +
  labs(x='Tmax (°C)', y='Corn yield [bu/ac]',
       title='Relation between temperature and yield(Tmax+Tmaxsq)')+
  geom_smooth(method=lm, color='blue', lty='dashed')
...
#Relationship for a given county(winneshiek)
...
ggplot(soybean_yields, mapping = aes(x=year, y=yield)) +
  geom_point() +
  geom_line (mapping = aes(x=year, y=fitted), col='red') +
  theme_bw() +
  labs(x="Year", y="Soybeans Yield [bu/ac]",
       title='Relation between Year and Soybean Yields in Winneshiek County')+
  geom_smooth(method=lm, color='blue', lty='dashed')
